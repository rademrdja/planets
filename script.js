...
var linkMob = document.querySelectorAll(".nav-link-mob");

var navBtn = document.querySelector("#nav-toggle");
var mobileNav = document.querySelector(".alternative-nav");

var planet = "mercury";

var image = document.querySelector(".planet-img");
var geologyImg = document.querySelector(".geology-img");
var wikiLink = document.querySelector(".wiki-link");

var overviewBtn = document.querySelector(".overviewbtn");
var structureBtn = document.querySelector(".structurebtn");
var geologyBtn = document.querySelector(".geologybtn");
var overviewBtnAlt = document.querySelector(".alt-overview");
var structureBtnAlt = document.querySelector(".alt-structure");
var geologyBtnAlt = document.querySelector(".alt-geology");

var heading = document.querySelector(".planet-name");

var rotationLbl = document.querySelector(".rotation");
var revolutionLbl = document.querySelector(".revolution");
var radiusLbl = document.querySelector(".radius");
var tempLbl = document.querySelector(".temp");
var description = document.querySelector(".description");

var currentPlanet;
var currentColor = "#419EBB";

navBtn.addEventListener("click", function () { // navigation action
...
structureBtn.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeStructure(planet);
});

structureBtnAlt.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeStructure(planet);
});
...
overviewBtn.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeOverview(planet);
});

overviewBtnAlt.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeOverview(planet);
});
...
geologyBtn.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeGeology(planet);
});

geologyBtnAlt.addEventListener("click", function () {
    currentPlanet = heading.textContent;
    changeGeology(planet);
});
...
function changeStructure(planet) {
    structureBtn.style.backgroundColor = currentColor;
    geologyBtn.style.backgroundColor = "transparent";
    overviewBtn.style.backgroundColor = "transparent";

    structureBtnAlt.style.borderBottom = "3px solid " + currentColor;
    overviewBtnAlt.style.borderBottom = "3px solid transparent";
    geologyBtnAlt.style.borderBottom = "3px solid transparent";

    for (var i = 0; i <= 7; i++) {
        if (data[i].name.toLowerCase() === planet) {
            description.textContent = data[i].structure.content;
            wikiLink.href = data[i].structure.source;
        }
    }

    geologyImg.style.display = "none";
    image.src = `images/planet-${planet}-internal.svg`;
}

function changeOverview(planet) {
    overviewBtn.style.backgroundColor = currentColor;
    structureBtn.style.backgroundColor = "transparent";
    geologyBtn.style.backgroundColor = "transparent";

    overviewBtnAlt.style.borderBottom = "3px solid " + currentColor;
    structureBtnAlt.style.borderBottom = "3px solid transparent";
    geologyBtnAlt.style.borderBottom = "3px solid transparent";

    for (var i = 0; i <= 7; i++) {
        if (data[i].name.toLowerCase() === planet) {
            description.textContent = data[i].overview.content;
            wikiLink.href = data[i].overview.source;
        }
    }

    geologyImg.style.display = "none";
    image.src = `images/planet-${planet}.svg`;
}

function changeGeology(planet) {
    geologyBtn.style.backgroundColor = currentColor;
    structureBtn.style.backgroundColor = "transparent";
    overviewBtn.style.backgroundColor = "transparent";

    geologyBtnAlt.style.borderBottom = "3px solid " + currentColor;
    structureBtnAlt.style.borderBottom = "3px solid transparent";
    overviewBtnAlt.style.borderBottom = "3px solid transparent";

    for (var i = 0; i <= 7; i++) {
        if (data[i].name.toLowerCase() === planet) {
            description.textContent = data[i].geology.content;
            wikiLink.href = data[i].geology.source;
        }
    }

    geologyImg.style.display = "block";
    image.src = `images/planet-${planet}.svg`
    geologyImg.src = `images/geology-${planet}.png`;
}

function changePlanet(planet) {
    image.src = `images/planet-${planet}.svg`;
    heading.textContent = planet.toUpperCase();
    structureBtn.style.backgroundColor = "transparent";
    geologyBtn.style.backgroundColor = "transparent";
    structureBtnAlt.style.borderBottom = "3px solid transparent";
    geologyBtnAlt.style.borderBottom = "3px solid transparent";
    ...
    for (var i = 0; i <= 7; i++) {
        if (colors[i].name === planet) {
            currentColor = colors[i].color;
            overviewBtn.style.backgroundColor = currentColor;
            overviewBtnAlt.style.borderBottom = "3px solid " + currentColor;
        }
        if (data[i].name.toLowerCase() === planet) {
            rotationLbl.textContent = data[i].rotation.toUpperCase();
            revolutionLbl.textContent = data[i].revolution.toUpperCase();
            radiusLbl.textContent = data[i].radius.toUpperCase();
            tempLbl.textContent = data[i].temperature.toUpperCase();
            description.textContent = data[i].overview.content;
            wikiLink.href = data[i].overview.source;
        }
    }
...
for (let i = 0; i <= 7; i++) {
    link[i].addEventListener("click", function () {
        planet = link[i].textContent.toLowerCase();
        changePlanet(planet);
    })
    linkMob[i].addEventListener("click", function () {
        planet = linkMob[i].textContent.toLowerCase().trim();
...
for (let i = 0; i <= 7; i++) {
    link[i].addEventListener("mouseenter", function () {
        if (colors[i].name === link[i].textContent.toLowerCase()) {
            link[i].style.borderTop = "4px solid " + colors[i].color;
        }
    });
    link[i].addEventListener("mouseleave", function () {
        link[i].style.borderTop = "4px solid transparent";
    });
}
